...
void Conjunto<T>::remover(const T& clave) {
    ...
    if(actual->izq == nullptr){
        transplantar(_raiz,actual,actual->der);
    }
    else if(actual->der == nullptr){
        transplantar(_raiz,actual,actual->izq);
    }
    else{
        Nodo* subMin= actual->der;
        Nodo* padreMin= nullptr;
        while(subMin->izq != nullptr){
            padreMin= subMin;
            subMin= padre->izq;
        }
        if(padre != actual){
        ...
        transplantar(_raiz,actual,subMin);
        subMin->izq= actual->der;
    ...
    if(actual != nullptr){//si es nill el elemento no esta, no tengo que hacer nada
        ...
        if(actual->der == nullptr && actual->izq == nullptr){
            ...
            T sig= siguienteNodo(_raiz,clave);
            Nodo* padresigPtr= nullptr;
            Nodo* sigPtr=_raiz;
            ...
            actual->valor= sig;
            //remover(clave);
            if(sigPtr->der == nullptr && sigPtr->izq == nullptr){
                if(esHijoIzquierdo){
                    padresigPtr->izq= nullptr;
                    delete sigPtr;
                }
                else{
                    padresigPtr->der= nullptr;
                    delete sigPtr;
                }
                //padresigPtr= nullptr;
                //delete sigPtr;
            }
            else if(sigPtr->der == nullptr && sigPtr->izq != nullptr){
                //delete sigPtr;
                if(esHijoIzquierdo){
                    padresigPtr->izq= sigPtr->izq;
                    delete sigPtr;
                }
                else{
                    padresigPtr->der= sigPtr->izq;
                    delete sigPtr;
                } 
                ...
                if(esHijoIzquierdo){
                    padresigPtr->izq= sigPtr->der;
                    delete sigPtr;
                }
                else{
                    padresigPtr->der= sigPtr->der;
                    delete sigPtr; 
                }
            }
        }
...
/*template <class T>
void Conjunto<T>::transplantar(Nodo* _raiz, Nodo* u, Nodo* v){
    Nodo* padreu= nullptr;
    Nodo* actual= _raiz;
    while(actual != u){
        if(actual->valor < u->valor){
            padreu= actual;
            actual= actual->izq;
        }
        else{
    ...
    if(v != nullptr ){
        Nodo* padreV= _raiz;
        while(padreV->izq == v|| padreV->der == v){
            if(padreV->valor < v->valor){
                padreV= padreV->der;
            }
            else{
                padreV= padreV->izq;
...
template<class T>
const T& Conjunto<T>::siguienteNodo(Nodo* arb, const T& elem){
    //pre: existe el siguiente nodo
    Nodo* actual= arb;
    Nodo* ultPadreIzquierdo= arb;
    while(actual->valor != elem){
        if(elem > actual->valor){
            actual= actual->der;
        }
        else{
            ultPadreIzquierdo= actual;
            actual= actual->izq;
        }
    }
    ...
    if(actual->der != nullptr){
        ...
        return ultPadreIzquierdo->valor;
    ...
    if(arb->valor <= elem){
        //buscar a la derecha
        siguienteNodo(arb->der,elem);
    }
    else{
        Nodo* siguiente= arb;
        //busco a la izquierda un valor menor
        while(siguiente->izq != nullptr && (siguiente->izq)->valor > elem){
            siguiente= siguiente->izq;
        }
        return siguienteNodo(arb,elem);
    }*/
...
unsigned int Conjunto<T>::contarRecursivo(Nodo* arb) const{
    ...
    if(arb == nullptr)
        ...
        if(arb->izq == nullptr && arb->der == nullptr)
            res= 1;
        else if(arb->izq == nullptr && arb->der != nullptr)
            res= 1+contarRecursivo(arb->der);
        else if(arb->izq != nullptr && arb->der == nullptr)
            res= 1+contarRecursivo(arb->izq);
        else if(arb->izq != nullptr && arb->der != nullptr)
            res= 1+contarRecursivo(arb->der)+contarRecursivo(arb->izq);
    }
    return res;
}
